...
def get_MELD_loaders(batch_size=32, valid=0.1, num_workers=0, pin_memory=False):
    trainset = MELDDataset('/data/dataset/MELD_features_raw1.pkl')
    ...
    testset = MELDDataset('/data/dataset/MELD_features_raw1.pkl', train=False)
...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, modals, optimizer=None, train=False, dataset='IEMOCAP'):
    ...
    avg_fscore = round(f1_score(labels,preds, average='weighted',zero_division=1)*100, 2)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num_M", type=int, default=3, help="num_hyperbolic_GCN")
    
    parser.add_argument("--curvature", type=float, default=0.1, help="initial curvature")
    ...
    best_fscore, best_loss, best_label, best_pred, best_mask, best_acc = None, None, None, None, None, None
    all_fscore, all_acc, all_loss = [], [], []

    if args.testing:
        state = torch.load("/data/baseline/Roberta_IEMOCAP1.pth.tar")
    ...
    for e in range(n_epochs):
        ...
        if best_fscore == None or best_fscore < test_fscore:
            ...
            #test_loss, test_acc, test_label, test_pred, test_fscore, _, _, _, _, _ = train_or_eval_graph_model(model, loss_function, test_loader, e, cuda, args.modals, dataset=args.Dataset)
        # elif best_acc == None or best_acc < test_acc:
        #     best_acc = test_acc
        #     best_label, best_pred = test_label, test_pred
        if args.tensorboard:
    ...
    if not args.testing:
        ...
        # YYYYMMDDHHMM 형식으로 포맷팅
        formatted_now = now.strftime("%Y%m%d%H%M")

        print(formatted_now)

        save_path = f"record_{args.graph_type}_num_K_{args.num_K}_num_L_{args.num_L}_num_M_{args.num_M}_curvature_{args.curvature}_dataset_{args.Dataset}_{formatted_now}.pk"
        ...
        with open(save_path, 'rb') as f:
            record = pk.load(f)
        key_ = name_
        record['acc']=round(accuracy_score(best_label,best_pred)*100, 2)
        record['f1']=round(f1_score(best_label,best_pred, average='weighted',zero_division=1)*100, 2)

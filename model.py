...
from model_hyperbolicGCNSingle import HyperbolicGCNSingle
from model_hyperbolicGCNSingle_highfreq import HyperbolicGCNSingle_highfreq
from model_hyperbolicGCNSingle_hypergraph import HyperbolicGCNSingle_hypergraph
from model_hyperbolicGCN_highfreq_hypergraph import HyperbolicGCN_highfreq_hypergraph
def print_grad(grad):
...
class Model(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future, 
                 ...
                 use_speaker=True, use_modal=False, norm='LN2', num_L = 3, num_K = 4, num_M=3, curvature=0.1):
        ...
        elif self.graph_type=="hyper_highfreq_hyperbolic":
            print("construct "+self.graph_type)
            self.graph_model = HyperGCN_hyperbolic(a_dim=D_g, v_dim=D_g, l_dim=D_g, n_dim=D_g, nlayers=64, nhidden=graph_hidden_size, nclass=n_classes, 
                                        dropout=self.dropout, lamda=0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue, n_speakers=n_speakers, modals=self.modals, use_speaker=self.use_speaker, use_modal=self.use_modal, num_L=num_L, num_K=num_K,num_M=num_M, curvature=curvature)
            
        elif self.graph_type=='hyperbolicGCN_highfreq_hypergraph':
            self.graph_model = HyperbolicGCN_highfreq_hypergraph(a_dim=D_g, v_dim=D_g, l_dim=D_g, n_dim=D_g, nlayers=64, nhidden=graph_hidden_size, nclass=n_classes, 
                                        dropout=self.dropout, lamda=0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue, n_speakers=n_speakers, modals=self.modals, use_speaker=self.use_speaker, use_modal=self.use_modal, num_L=num_L, num_K=num_K,curvature=curvature)
            print("construct "+self.graph_type)
            
        elif self.graph_type=='hyperbolicGCNSingle_hypergraph':
            self.graph_model = HyperbolicGCNSingle_hypergraph(a_dim=D_g, v_dim=D_g, l_dim=D_g, n_dim=D_g, nlayers=64, nhidden=graph_hidden_size, nclass=n_classes, 
                                        dropout=self.dropout, lamda=0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue, n_speakers=n_speakers, modals=self.modals, use_speaker=self.use_speaker, use_modal=self.use_modal, num_L=num_L, num_K=num_K,curvature=curvature)
            print("construct "+self.graph_type)
            
        
        elif self.graph_type=="hyperbolicGCNSingle":
            self.graph_model = HyperbolicGCNSingle(a_dim=D_g, v_dim=D_g, l_dim=D_g, n_dim=D_g, nlayers=64, nhidden=graph_hidden_size, nclass=n_classes, 
                                        dropout=self.dropout, lamda=0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue, n_speakers=n_speakers, modals=self.modals, use_speaker=self.use_speaker, use_modal=self.use_modal, num_L=num_L, num_K=num_K,curvature=curvature)
            print("construct "+self.graph_type)
        elif self.graph_type=="hyperbolicGCNSingle_highfreq":
            self.graph_model = HyperbolicGCNSingle_highfreq(a_dim=D_g, v_dim=D_g, l_dim=D_g, n_dim=D_g, nlayers=64, nhidden=graph_hidden_size, nclass=n_classes, 
                                        dropout=self.dropout, lamda=0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue, n_speakers=n_speakers, modals=self.modals, use_speaker=self.use_speaker, use_modal=self.use_modal, num_L=num_L, num_K=num_K,curvature=curvature)
            print("construct "+self.graph_type)
        ...
        if self.multi_modal:
            ...
            if self.att_type == 'concat_subsequently':
                ...
                if self.use_residue:
                    ...
                    if self.graph_type == "hyperbolicGCN":
                        self.smax_fc = nn.Linear((graph_hidden_size*2)*len(self.modals)+len(self.modals), n_classes)
                    elif self.graph_type == "hyper_highfreq_hyperbolic":
                        self.smax_fc = nn.Linear((graph_hidden_size*3)*len(self.modals)+len(self.modals), n_classes)
                    elif self.graph_type == "hyper":
                        self.smax_fc = nn.Linear((graph_hidden_size*2)*len(self.modals), n_classes)  
                    elif self.graph_type == "hyperbolicGCNSingle":
                        self.smax_fc = nn.Linear((graph_hidden_size)*len(self.modals)+len(self.modals), n_classes)
                    elif self.graph_type == "hyperbolicGCNSingle_highfreq":
                        self.smax_fc = nn.Linear((graph_hidden_size)*len(self.modals)+len(self.modals), n_classes)
                    elif self.graph_type == "hyperbolicGCNSingle_hypergraph":
                        self.smax_fc = nn.Linear((graph_hidden_size)*len(self.modals)+len(self.modals), n_classes)
                    elif self.graph_type == "hyperbolicGCN_highfreq_hypergraph":
                        self.smax_fc = nn.Linear((graph_hidden_size*2)*len(self.modals)+2*len(self.modals), n_classes)
                    elif self.graph_type == "hyperbolicGCN_highfreq_hypergraph":
                        self.smax_fc = nn.Linear((graph_hidden_size*2)*len(self.modals)+2*len(self.modals), n_classes)
    ...
    def forward(self, U, qmask, umask, seq_lengths, U_a=None, U_v=None, epoch=None):
        ...
        elif self.graph_type=='hyperbolicGCN':
            emotions_feat = self.graph_model(features_a, features_v, features_l, seq_lengths, qmask, epoch)

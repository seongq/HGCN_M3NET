...
class HyperbolicGCNSingle_hypergraph(nn.Module):
    def __init__(self, a_dim, v_dim, l_dim, n_dim, nlayers, nhidden, nclass, dropout, lamda, alpha, variant, return_feature, use_residue, 
        ...
        super(HyperbolicGCNSingle_hypergraph, self).__init__()
        ...
        setattr(self,'hyperconv%d' , hyperbolicGCN_hypergraph(self.num_K, nhidden+1, nhidden+1,dropout_rate= self.dropout, curvature=curvature))
        self.act_fn = nn.ReLU()
    ...
    def forward(self, a, v, l, dia_len, qmask, epoch):
        ...
        ##encode
        # o = torch.zeros_like(gnn_out)
        # # print(gnn_out.size())
        # # print(o.size())
        # gnn_out = torch.cat([o[:,0:1],gnn_out], dim=1)

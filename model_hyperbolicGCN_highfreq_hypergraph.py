...
from HyperbolicGCN_highfreq import hyperbolicGCN_highfreq
from HyperbolicGCN_hypergraph import hyperbolicGCN_hypergraph
...
class HyperbolicGCN_highfreq_hypergraph(nn.Module):
    def __init__(self, a_dim, v_dim, l_dim, n_dim, nlayers, nhidden, nclass, dropout, lamda, alpha, variant, return_feature, use_residue, 
                new_graph='full',n_speakers=2, modals=['a','v','l'], use_speaker=True, use_modal=False, num_L=3, num_K=4, curvature=0.1):
        super(HyperbolicGCN_highfreq_hypergraph, self).__init__()
        ...
        setattr(self,'hyperconv%d' , hyperbolicGCN_hypergraph(self.num_L, nhidden+1, nhidden+1, dropout_rate=self.dropout, curvature=curvature))
        self.act_fn = nn.ReLU()
        ...
        setattr(self, "hyperbolicgconv", hyperbolicGCN_highfreq(self.num_K, nhidden+1, nhidden+1, dropout_rate = self.dropout, curvature=curvature))
    ...
    def forward(self, a, v, l, dia_len, qmask, epoch):
        ...
        out = x1
        out = getattr(self,'hyperconv%d')(out, hyperedge_index, weight, edge_attr, EW_weight, dia_len)    
        # print(self.use_residue)         
        # if self.use_residue:
        ...
        gnn_out =  getattr(self,'hyperbolicgconv')(gnn_out,gnn_edge_index)

       
        out2 = torch.cat([out,gnn_out], dim=1)
        out = self.reverse_features(dia_len, out2)
        return out
